...
- **Local roadmap** — gitignored root `checklist.md` (not deployed); update as work ships.

After editing architecture, changelog, or `README_HEADER.md`, run `python scripts/sync_docs.py`. Interactive pipeline: visitor copy in `docs/assets/pipeline-config.json`; technical copy in `docs/assets/pipeline-config-developer.json` (referenced from `ARCHITECTURE_DEVELOPER.md`).
