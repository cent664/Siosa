...
- **Topic-switch gate** — prior pages / link expand / history search hints apply only when the Ask continues the prior topic (deixis or shared keywords). A new topic (e.g. poison after Pantheon) gets a fresh live wiki search. Click **Siosa's Library** in the UI to clear the session and start over.
- **Wiki redirects + search-first** — parse uses `redirects=1` (`Pantheon` → `The Pantheon`); fetch budget prefers MediaWiki search hits over invented title probes; trace shows fetch/search errors. Fixes empty cloud answers when local only “worked” via a tiny cached stub.
